...
import os
import mc
sys.path.append('py/HGuild_Runtime')
import json
import yaml
PluginVersion = 2.5
##################################################################################################
...
#检测运行库
if not os.path.exists('py/HGuild_Runtime'):
...
def language_read():
    ...
    Plugins_Path= 'py/HGuild_Runtime/GuildConfig'
    try:
        with open(Plugins_Path+'/Configuration.yml','r',encoding='utf-8') as f:
            Config = yaml.load(f.read(), Loader=yaml.FullLoader)
    except:
        pass
    if os.path.exists('py/HGuild_Runtime/GuildConfig/language.yml'):
        with open('py/HGuild_Runtime/GuildConfig/language.yml','r',encoding='utf-8') as f:
            ...
            if 'language_type' not in language_file:
                download('http://mchuohua.gitee.io/HGuild/Language/'+Config['Language']+'.yml','py/HGuild_Runtime/GuildConfig/language.yml')
...
def createFile(Filename,content):
    lsa = 'py/HGuild_Runtime/GuildConfig/'+Filename
    try:
        if not os.path.exists(lsa):
            with open('./py/HGuild_Runtime/GuildConfig/'+Filename,'w',encoding='utf-8') as f:
                f.write(content)
...
def Self_test():
    try:
        #检查文件夹
        createFolder('py/HGuild_Runtime/GuildConfig')
        #检查Config
...
#EULA协议
def Test_Eula():
    if not os.path.exists('py/HGuild_Runtime/GuildConfig/eula'):
        download('http://mchuohua.gitee.io/HGuild/eula','py/HGuild_Runtime/GuildConfig/eula')
        print('[HGuild] '+language_file['eula'])
        mc.runcmd('ds')
    else:
        with open('py/HGuild_Runtime/GuildConfig/eula','r',encoding='utf-8') as f:
            elua = f.readlines()
...
#插件升级
def plugin_update():
    if Config['YamlVersion'] != 3:
        with open('py/HGuild_Runtime/GuildConfig/Configuration.yml','w',encoding='utf-8') as f:
            f.write(yaml.dump(
            {   
                "Config":{
                    "Language":"zh_cn",
    ...
    if float(req['HGuildMVersion']) > PluginVersion:
        if req['Update_type'] == 'Auto':
            ...
            download('http://mchuohua.gitee.io/HGuild/guild.pyd','guild.pyd')
            try:
                os.remove('./py/HGuild_Runtime/guild.pyd')
            except:
                pass
            try:
                shutil.move('guild.pyd','./py/HGuild_Runtime/guild.pyd')
            except:
                pass
            try:
                os.remove('guild.pyd')
...
def Write_File(FileType):
    ...
    if FileType == 'Guild':
        with open('py/HGuild_Runtime/GuildConfig/GuildFile.yml','w') as f:
            GuildConfigDict = GuildConfig
            Guilds = yaml.dump(GuildConfigDict)
            f.write(Guilds)
    elif FileType == 'Player':
        with open('py/HGuild_Runtime/GuildConfig/PlayerConfig.yml','w') as f:
            Players = yaml.dump(PlayerConfig)
            f.write(Players)
    elif FileType == 'Check':
        with open('py/HGuild_Runtime/GuildConfig/Checked.yml','w') as f:
...
def Send_Error(player,title,content):
    mc.sendModalForm(player, language_file["Gui"]["Error_Gui"]["Title_Error"]+title, '§e'+content, language_file["Gui"]["Error_Gui"]["OK"],language_file["Gui"]["Error_Gui"]["Cancel"])
##################################################################################################
#公会签到
def Sign_In():
...
#开启线程
try:
    WriteInfo('[HGuild]',language_file['Timer_Open'])
    mc.setTimer(Sign_In,1)
    #创建计分板
except:
    WriteError('[INFO]',language_file['Timer_Error'])
##################################################################################################
n = 0
#玩家加入
def Player_Join(e):
...
def input_cmd(e):
    global main
    try:
        if e['cmd'][0] == '/'+Config['Cmd']:
            main = mc.sendCustomForm(e['player'],'{"content": "'+language_file['Gui']['Send_Main']+'","buttons":'+json.dumps(mainmenu)+', "type":"form","title":"§b§lHGuild §cMenu"}')
            return False
...
def Guild_Members(player,guild):
    ...
    mc.sendSimpleForm(player,language_file['Gui']["Guild_Member_Text"]['Gui_Title'][0]+guild+language_file['Gui']["Guild_Member_Text"]['Gui_Title'][1],Members,'[]')
##################################################################################################
#表单操作
def form_action(e):
    ...
    if e['formid'] == main:
        ...
        if e['selected'] == 0:
            ...
            if scores >= int(Config['Cost']):
                if PlayerConfig[e['playername']]['Created_Guild'] == 0:
                    ...
                    create_Guild_id = mc.sendCustomForm(e['player'],json.dumps(createGuild))
                else:
        ...
        #main表单公会管理反馈
            ...
            if PlayerConfig[e['playername']]['Join'] != 'Null':
                ...
                Manage_Guild_Gui = mc.sendCustomForm(e['player'],'{"content": "'+language_file['Gui']['Guild_Manage']["Send_Gui"]+'","buttons":'+json.dumps(clos)+', "type":"form","title":"'+language_file['Gui']['Guild_Manage']["Title"]+'"}')
            else:
        ...
        #main表单传送反馈
            ...
            if Teleport_Guild_Name != 'Null':
                x = int(GuildConfig[Teleport_Guild_Name]['Teleport']['x'])
                y = int(GuildConfig[Teleport_Guild_Name]['Teleport']['y'])
                z = int(GuildConfig[Teleport_Guild_Name]['Teleport']['z'])
                dim = int(GuildConfig[Teleport_Guild_Name]['Teleport']['dim'])
                mc.teleport(e['player'],x,y,z,dim)
        ...
        #main表单世界公会反馈
            ...
            World_Guild_Gui = mc.sendSimpleForm(e['player'],language_file['Gui']["World_Guild_Text"]['Gui_Title'][0],language_file['Gui']["World_Guild_Text"]['Gui_Title'][1],json.dumps(World_Guild_List_Admin))
##################################################################################################
    #Create_Guild创建公会反馈
    ...
    #Manage_Guild_Gui公会管理
        ...
        if PlayerConfig[e['playername']]['Permission'] == 'Admin':
            ...
            #公会管理--公会改名
                ...
                Change_Name_Gui = mc.sendCustomForm(e['player'],json.dumps(Change_Name))
            ...
            #公会管理--邀请玩家
                ...
                InvitationPlayer_Gui = mc.sendCustomForm(e['player'],json.dumps(contents))
            ...
            #公会管理--更改权限
                ...
                Change_Permission_Gui = mc.sendCustomForm(e['player'],json.dumps(Change_Permission))
            #公会管理--踢出玩家
            elif e['selected'] == 4:
                ...
                Kick_Player_Gui_Id = mc.sendCustomForm(e['player'],json.dumps(Kick_Player_Gui))
            ...
            #公会管理--更改传送
                ...
                Change_Position_Gui_Id = mc.sendCustomForm(e['player'],json.dumps(Change_Position_Gui))
            ...
            #公会管理--解散公会
                ...
                Dissolution_Guild_Gui_Id = mc.sendCustomForm(e['player'],json.dumps(Dissolution_Guild_Gui))
        #管理版管理
        elif PlayerConfig[e['playername']]['Permission'] == 'Operator':
            try:
                e['selected'] = int(e['selected'])
    ...
    #公会管理操作--邀请玩家
        ...
        Invitation_Send_Gui_Id = mc.sendModalForm(player,language_file['Invitation_Player_Action'][0]+Invitation_Guild_Name+language_file['Invitation_Player_Action'][1],language_file['Invitation_Player_Action'][2]+e['selected'][1],language_file['Invitation_Player_Action'][3],language_file['Invitation_Player_Action'][4])
    ...
    #世界公会--选择公会
        ...
        for i in GuildConfig[Selected_Guild]['Member']:
            if GuildConfig[Group_Member_Name]['Member'][i] == 'Admin':
                ...
                Members += language_file['Gui']["Guild_Member_Text"]["Member"]+i
        World_Guild_Selected_Id = mc.sendSimpleForm(e['player'],language_file['Gui']["Guild_Member_Text"]['Gui_Title'][0]+Selected_Guild+language_file['Gui']["Guild_Member_Text"]['Gui_Title'][1],Members,'[{"text":"'+language_file['Gui']["Guild_Member_Text"]['Join_Them']+'"}]')
    ...
    #世界公会加入操作
        ...
        if e['selected'] == '1':
            if PlayerConfig[e['playername']]['Join'] == 'Null':
                World_Guild_accept = mc.sendModalForm(e['player'],language_file['World_Guild_Selected_Action']['Ask'][0],language_file['World_Guild_Selected_Action']['Ask'][1],language_file['World_Guild_Selected_Action']['Ask'][2],language_file['World_Guild_Selected_Action']['Ask'][3])
            else:
    ...
    #移除线程 
    mc.removeTimer(form_action)
#监听数据
def form_selected(e):
    #判断数据
    if e['selected'] != 'null':
        #创建线程
        mc.setTimer(form_action(e),1)
...
def load_plugin():
    ...
    if Py_Version == 3:
        ...
        Server_Cmd = '后台输入'
        Select_Form = '选择表单'
        Player_Input_Cmd = '输入指令'
        Player_Join_Game = '加入游戏'
        #玩家攻击监听器
        try:
            mc.setListener(player_pvp,Pvp)
            WriteInfo('[HGuild]','监听器:'+player_pvp+' 注册成功')
        except:
            WriteError('[HGuild]','监听器:'+player_pvp+' 注册失败')
        #后台输入监听器
        try:
            mc.setListener(Server_Cmd,listen_cmd)
            WriteInfo('[HGuild]','监听器:'+Server_Cmd+' 注册成功')
        except:
            WriteError('[HGuild]','监听器:'+Server_Cmd+' 注册失败')
        #选择表单监听器
        try:
            mc.setListener(Select_Form,form_selected)
            WriteInfo('[HGuild]','监听器:'+Select_Form+' 注册成功')
        except:
            WriteError('[HGuild]','监听器:'+Select_Form+' 注册失败')
        #输入指令监听器
        try:
            mc.setListener(Player_Input_Cmd,input_cmd)
            WriteInfo('[HGuild]','监听器:'+Player_Input_Cmd+' 注册成功')
        except:
            WriteError('[HGuild]','监听器:'+Player_Input_Cmd+' 注册失败')
        #加入游戏监听器
        try:
            mc.setListener(Player_Join_Game,Player_Join)
            WriteInfo('[HGuild]','监听器:'+Player_Join_Game+' 注册成功')
        except:
            WriteError('[HGuild]','监听器:'+Player_Join_Game+' 注册失败')
        #插件更新
        try:
            mc.setTimer(plugin_update,1)
        except:
            WriteError('[HGuild]','调用检测更新函数时发生线程错误！')
    else:
        WriteError('[HGuild]',language_file["Loader_Error"])

    #注册命令
    try:
        mc.setCommandDescription(Config['Cmd'],language_file["Cmd_Title"])
        WriteInfo('[HGuild]',language_file["Register_Cmd_Success"][0]+Config['Cmd']+language_file["Register_Cmd_Success"][1])
    except:
        WriteError('[HGuild]',language_file["Register_Cmd_Fail"])
    Test_Eula()
